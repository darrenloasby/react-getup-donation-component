  ...
  return Object.entries(inputs)
    .map(([name, value]) => {
      return value === undefined
        ? null
        : `<input type="hidden" name="${htmlEntities(
            name
...
class SecureFrame extends Component {
  ...
  onPayment(response) {
    if (
      response.summarycode === SUMMARYCODE_APPROVED &&
      this.props.onPaymentApproved
    ) {
      return this.props.onPaymentApproved(response);
    }
    if (
      [SUMMARYCODE_DECLINEDBYBANK, SUMMARYCODE_DECLINEDOTHER].includes(
    ...
    if (
      response.summarycode === SUMMARYCODE_CANCELLED &&
  ...
  makeFingerprint(_props) {
    ...
    return props.onSignPayment(subject, data).then((response) => {
  ...
  render() {
    ...
    const inputs = {
      ...
      page_style_url:
        styleUrl || "https://try.gency.com.au/tim/secureframe-react/style.css",
      return_url:
        returnUrl || "https://try.gency.com.au/tim/secureframe-react/",
    };

    const transactionUrl =
      this.props.transactionUrl || this.props.live
